...
contextBridge.exposeInMainWorld("ipcRenderer", {
  ...
  syncEmails: (arg: string | { accountId: string; path: string }) =>
    ipcRenderer.invoke("email:sync", arg),
  ...
  // [NEW] AI Methods exposed to Renderer
  aiHasKey: () => ipcRenderer.invoke("ai:hasKey"),
  aiSaveKey: (key: string) => ipcRenderer.invoke("ai:saveKey", key),
  aiSummarize: (body: string) => ipcRenderer.invoke("ai:summarize", body),
  aiDraft: (body: string, instruction: string) =>
    ipcRenderer.invoke("ai:draft", { body, instruction }),
});
